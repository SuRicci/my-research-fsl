...
import hashlib,json
import numpy as np
P=Path(__file__).resolve().parents[2]/'main/view-distribution-kernel-20260913'; O=P/'outputs'
...
def audit(a,names,seeds,saved,label):
    i=names.index('distribution_rbf')
...
for source,target in [('dtd','eurosat'),('eurosat','dtd')]:
    ...
    for shot in cfg['shots']:
        for gallery in [target,source]:
            ...
            old,mean,scatter=[a[names.index(m)] for m in ['original_rbf','mean_rbf','distribution_rbf']]
            rows[name]={'mean_rbf_minus_original_rbf':ci(mean-old,seeds),'distribution_minus_mean_rbf':ci(scatter-mean,seeds),'distribution_minus_mean_per_seed_pp':{str(seed):float((scatter-mean)[seeds==seed].mean()*100) for seed in np.unique(seeds)}}
...
assert len(checks)==130
result={'status':'passed_statistics_only','statistic_comparisons':len(checks),'max_stat_error':max(c['max_stat_error'] for c in checks),'cells':rows,'input_sha256':hashes,'scope':'Post-hoc fixed-pool decomposition. Bootstrap weights independently built with multinomial multiplicities; all130 predefined mean/interval comparisons recovered. Does not override failed dense end-to-end score audit, establish family impossibility, or add Pets/Caltech evidence.'}
