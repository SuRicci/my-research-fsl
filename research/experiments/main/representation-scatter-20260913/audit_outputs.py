...
def compare(actual,expected,label,errors):
    ...
    assert np.array_equal(actual.argmax(-1),expected.argmax(-1)),('argmax',label,error)
    errors.append({'label':label,'max_score_error':error})
...
def main():
    ...
    result={'status':'passed','task_conditions':task_conditions,'methods_per_cell':len(e.METHODS),'source_gamma_selections':selections,'source_and_eval_numpy_score_checks':len(errors),'candidate_sampled_eval_tasks':candidate_samples,'max_numpy_score_error':max(r['max_score_error'] for r in errors),'historical_original_score_max_error':historical_error,'all_source_predictions_equal_pre_repair':True,'score_tolerance':TOL,'frozen_gate_result':summary['status'],'elapsed_seconds':time.time()-started,'input_sha256':paths,'score_checks':errors,'scope':'All aggregate values, source choices, tasks, original control scores verified; dense independent candidate scores on48source cases and40evaluation tasks. Reuses verified cache loader. Bootstrap interval algorithm not independently reimplemented. Fixed-pool development evidence only.'}
    (OUT/'output_audit.json').write_text(json.dumps(result,indent=2))
