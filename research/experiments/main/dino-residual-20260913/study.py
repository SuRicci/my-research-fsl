"""Fixed residual descriptor qualification; source development only."""
...
def parts(q,s):
    ...
    s=norm(torch.as_tensor(np.array(s),dtype=torch.float64))
    center=s.mean((0,1,2))
    qr=norm(q-q.mean(-2,keepdim=True)); sr=norm(s-s.mean(-2,keepdim=True))
    qc=norm(q-center); sc=norm(s-center)
    def match(a,b):
        sim=a.unsqueeze(0)@b.flatten(1,2)[:,None].transpose(-1,-2)
        return sim.max(-1).values.mean(-1).T.numpy()
    return match(qr,sr),match(qc,sc)

def reference(q,s):
    def unit(x):
        size=np.linalg.norm(x,axis=-1,keepdims=True)
        return np.where(size>EPS,x/np.maximum(size,EPS),0.)
    q=unit(np.asarray(q,dtype=np.float64)); s=unit(np.asarray(s,dtype=np.float64))
    center=sum((v for image in s for shot in image for v in shot),np.zeros(q.shape[-1]))/(s.shape[0]*s.shape[1]*s.shape[2])
    def own(x):
        flat=x.reshape(-1,x.shape[-2],x.shape[-1]); out=[]
        for image in flat:
            mu=sum(image,np.zeros(image.shape[-1]))/len(image)
            out.append(unit(image-mu))
        return np.asarray(out).reshape(x.shape)
    def match(a,b):
        b=b.reshape(5,-1,b.shape[-1]); out=np.zeros((len(a),5))
        for i,image in enumerate(a):
            for c,support in enumerate(b):
                out[i,c]=sum(max(float(np.dot(v,t)) for t in support) for v in image)/len(image)
        return out
    return match(own(q),own(s)),match(unit(q-center),unit(s-center))

def scores(parent,raw,mean,part,rgb):
    residual,common=part; p=standard(parent)
    return np.stack([parent,residual,(p+standard(residual))/2,raw,(p+standard(common))/2,(p+standard(residual[:,shuffle(rgb)]))/2,mean])
...
def synthetic():
    rng=np.random.RandomState(260913103); q=rng.randn(3,16,24); s=rng.randn(5,1,16,24)
    p=rng.randn(3,5); raw=rng.randn(3,5); mean=rng.randn(3,5); rgb=list('abcde')
    checks=[{'case':'random',**validate(q,s,p,raw,mean,rgb)}]
    for name,qq,ss in [('zero',np.zeros_like(q),np.zeros_like(s)),('constant',np.ones_like(q),np.ones_like(s)),('tiny',np.ones_like(q)+1e-14*q,np.ones_like(s)+1e-14*s)]:
        a=parts(qq,ss); assert all(np.isfinite(x).all() for x in a)
        if name in ['zero','constant']: assert all(np.count_nonzero(x)==0 for x in a)
        # Tiny residuals test the fixed epsilon arithmetic, not rank stability at ties.
        b=reference(qq,ss); error=max(float(abs(x-y).max()) for x,y in zip(a,b)); assert error<1e-10,error
        checks.append({'case':name,'matching_error':error})
...
def main():
    ...
    code={str(p):sha(p) for p in [HERE/'study.py',HERE/'protocol.json',HERE/'verify.py']}; dump('code_lock.json',code)
    inputs={}; outputs={}; total=0
    for d in CFG['domains']:
        path=ASSET/(d+'_query.npy'); h=sha(path); assert h==manifest['pools'][d+'_query']['sha256']; inputs[str(path)]=h
        ident_path=ASSET/(d+'_query_identities.json'); ident=json.loads(ident_path.read_text()); inputs[str(ident_path)]=sha(ident_path)
        ...
        for i in range(500):
            ...
            if i%100==0: guard(); print('MATCHING',d,i,round(time.time()-start,2),flush=True)
        for g in CFG['domains']:
