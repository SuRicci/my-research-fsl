...
def result():
    ...
    for path in sorted(out.glob("*.npz")):
        ...
        names=list(a["names"]);bn=list(b["names"]);i=names.index("centered_r2");j=names.index("r2")
    ...
    names=per_dataset["dtd"][2];ci=names.index("centered_r2")
    for method in names:
    ...
    main=macro["r2"];worst=min(v["delta_vs_r2_pp"] for k,v in tables.items())
    strong={m:macro[m]["paired_ci95_pp"][0]>0 for m in ["r2_tuned","centered_support","centered_proto","support_ridge","support_proto","gallery_r2"]}
    ...
    lines=["# Paired experiment result","",report["scope"],"","| Condition | R2 % | Support-centered R2 % | Delta pp | 95% paired CI pp |","|---|---:|---:|---:|---|"]
    for k,v in tables.items():lines.append("| %s | %.3f | %.3f | %+.3f | [%.3f, %.3f] |"%(k,v["accuracy_percent"]["r2"],v["accuracy_percent"]["centered_r2"],v["delta_vs_r2_pp"],*v["paired_ci95_pp"]))
