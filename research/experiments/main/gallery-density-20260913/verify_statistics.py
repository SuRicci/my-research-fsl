...
def main():
    ...
    for domain in CFG['domains']:
        ...
        for gallery in CFG['domains']:
            name=domain+'_'+gallery;b=np.load(OUT/(name+'.npz'));scores=b['scores'];assert scores.shape==(4,500,75,5) and np.isfinite(scores).all()
            pred=scores.argmax(-1);acc=(pred==b['yq']).mean(-1);arrays.append(acc);n+=pred.size
            assert b['names'].tolist()==['parent','local','global','shuffled']
            for j,label in [(0,'parent'),(2,'global'),(3,'shuffled')]:compare(acc[1]-acc[j],b['seeds'],result['cells'][name]['comparisons'][label])
        ...
        for j,label in [(0,'parent'),(2,'global'),(3,'shuffled')]:compare(avg[1]-avg[j],b['seeds'],result['domains'][domain]['comparisons'][label])
    ...
    for j,label in [(0,'parent'),(2,'global'),(3,'shuffled')]:compare(acc[1]-acc[j],groups,result['pooled'][label])
