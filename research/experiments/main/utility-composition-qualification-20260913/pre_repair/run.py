...
def tasks(ident, shot, phase):
    ...
    for seed in seeds:
        ...
        for _ in range(n):
            ...
            ix=np.stack([rng.choice(pools[int(c)],shot+15,replace=False) for c in cs])
...
def batch(data, t, G, sl=slice(None)):
    ...
    Q=data['query'][t['query_indices'][sl].reshape(-1,75)]
...
def quality(cache,Q,c,a,uniform=False):
    ...
    ce=F.cross_entropy((CFG['query_logit_scale']*sc).flatten(0,1),y.repeat(len(Q)))
    acc=(sc.argmax(-1)==y).float().mean()
    return ce,acc
...
def learn(source,shot,data):
    ...
    for k in range(0,len(V),8):
        ...
        for key,sc in old.controls(S,V[k:k+8],G).items():accs.setdefault(key,[]).extend((sc.argmax(-1)==y.numpy()).mean(-1).tolist())
...
def main():
    ...
    if args.phase=='check':
        result=utility.validate(ref);dump('implementation_validation.json',result);old.dump(HERE/'locked_sources.json',hashes);print(result);return
    ...
    for source,target in [('dtd','eurosat'),('eurosat','dtd')]:
        for shot in [1,5]:
            m=learn(source,shot,data[source])
            for gallery in [target,source]:evaluate(source,target,gallery,shot,data,m)
