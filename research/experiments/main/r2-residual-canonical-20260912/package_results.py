...
def validate(path):
    ...
    return a
def baseline():
    out=HERE/"outputs/baseline"
    vals=json.loads((out/"metrics_summary.json").read_text());assert len(vals)==7
    for path in sorted(out.glob("*.npz")):validate(path)
    metrics=[]
    for key,value in vals.items():
        assert np.isfinite(value)
        metrics.append({"metric_id":key,"direction":"higher_is_better","unit":"%","required":True,
            "description":"Equal-cell mean accuracy over four1-shot query/gallery conditions" if key=="macro_1shot_accuracy" else "Mean 5-way episode query accuracy for "+key,
            "derivation":"100 × mean episode accuracy;1000 episodes per condition,5seeds×200;macro is unweighted mean of four1-shot cells.",
            "source_ref":str(out/"metrics_summary.json"),"origin_path":key})
    contract={"contract_id":"r2-canonical-local-mps32-v1","primary_metric_id":"macro_1shot_accuracy","metrics":metrics,
        "evaluation_protocol":{"scope_id":"dtd-eurosat-canonical-locked-20260912","feature_variant":CFG["feature_variant"],"protocol":CFG,
            "code_paths":[str(HERE/"evaluate.py"),str(HERE/"baseline_methods.py")],
            "code_hashes":{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [HERE/"evaluate.py",HERE/"baseline_methods.py"]},
            "legacy_reference":"r2-frozen-local DTD5 cached replay; not comparable with this encoder variant"}}
    target=Path(CFG["asset_root"]).parent/"json";target.mkdir(exist_ok=True)
    dump(target/"metric_contract.json",contract);dump(target/"metrics_summary.json",vals)
    dump(target/"baseline_verification.json",{"status":"passed","cells":6,"episodes_per_cell":1000,"raw_outputs":str(out),
        "support_query_disjoint":True,"scores_predictions_accuracy_recomputed":True,"feature_manifest":CFG["asset_root"]+"/manifest.json","historical_image_parity":False})
    print(json.dumps({"metrics_summary":vals,"contract":str(target/"metric_contract.json")}))
...
def result():
    out=HERE/"outputs/eval";base=HERE/"outputs/baseline";tables={};raw={};values={}
    for path in sorted(out.glob("*.npz")):
        ...
        names=list(a["names"]);bn=list(b["names"]);i=names.index("residual");j=names.index("r2")
    ...
    names=per_dataset["dtd"][2];ci=names.index("residual")
    for method in names:
    ...
    strong={m:macro[m]["paired_ci95_pp"][0]>0 for m in ["support_tuned","support_proto","isotropic"]}
    ...
    lines=["# Paired experiment result","",report["scope"],"","| Condition | R2 % | Residual % | Delta pp | 95% paired CI pp |","|---|---:|---:|---:|---|"]
    for k,v in tables.items():lines.append("| %s | %.3f | %.3f | %+.3f | [%.3f, %.3f] |"%(k,v["accuracy_percent"]["r2"],v["accuracy_percent"]["residual"],v["delta_vs_r2_pp"],*v["paired_ci95_pp"]))
    lines+=["","Decision gate: "+str(gate),"","Macro control comparison:",json.dumps(macro,indent=2)]
    (out/"RESULT.md").write_text("\n".join(lines))
    print(json.dumps({"gate":gate,"macro_vs_controls":macro,"metrics_summary":values},indent=2))
if __name__=="__main__":
    p=argparse.ArgumentParser();p.add_argument("--mode",choices=["baseline","result"],required=True)
    baseline() if p.parse_args().mode=="baseline" else result()
