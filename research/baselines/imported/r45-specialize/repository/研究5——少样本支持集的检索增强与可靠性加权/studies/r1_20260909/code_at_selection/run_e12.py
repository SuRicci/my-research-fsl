...
ROOT=HERE.parent
sys.path.insert(0,str(next(ROOT.glob('实验12*'))))
...
def evaluate_state(qstate,gstate,es,glabels,gnames,qnames,phase,cell,b,shot):
    ...
    old=json.loads((next(ROOT.glob('实验12*'))/'results'/'pilot_main.json').read_text(encoding='utf-8'))
...
def summarize():
    ...
    for path in sorted(OUT.glob('*_retest.npz')):
        ...
        for a,ref in [('weighted_mix','uniform_mix'),('uniform_gate','uniform_mix'),('weighted_gate','uniform_gate'),('weighted_gate','uniform_mix'),('weighted_gate','raw')]:
