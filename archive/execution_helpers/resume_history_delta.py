...
C=T/'history-delta-recovery'
assert C.exists() and git(C,'rev-list','--count',BASE+'..HEAD').strip()=='5','Resume only the already applied five-commit recovery check'
print('RESUMING_COMPLETED_PATCH_APPLICATION',flush=True)
rebuilt=git(C,'rev-parse','HEAD').strip();tree=git(C,'rev-parse','HEAD^{tree}').strip()
...
print('DELTA_CONTENT_HASHES_AND_CONNECTIVITY_PASSED',flush=True)
...
receipt={'status':'passed','scope':'Five archive-administration commits created after the frozen scientific history; no experiment, baseline, paper or literature files changed','commit_count':5,'original_base':OLD,'original_head':NEW,'sanitized_base':BASE,'affected_branch':BRANCH,'source_commits':[x.split()[0] for x in commits],'independent_patch_application':'passed','validation_note':'All changed file bytes compared to originals; connectivity checked. Unchanged base objects retain the prior full-fsck audit; full base scan interrupted at 60 seconds and was not repeated.','reconstructed_head_example':rebuilt,'reconstructed_tree':tree,'changed_files_sha256':checks,'credential_pattern_hits':0,'patches':[{'file':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in patches],'verified_at':datetime.datetime.now(datetime.timezone.utc).isoformat()}
